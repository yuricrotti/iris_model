...
def model_monitoring(config_path):
    ...
    print('New Train Data Path: ',new_train_data_path)
    print('Target: ',target)

    monitor_dashboard_path = config["model_monitor"]["monitor_dashboard_html"]
    monitor_target = config["model_monitor"]["target_col_name"]
    print('Monitor Dashboard Path: ',monitor_dashboard_path)
    print('Monitor Target: ',monitor_target)

    ref=pd.read_csv(train_data_path)
    cur=pd.read_csv(new_train_data_path)
    print('Data Loaded')
    print('Reference Data Shape: ',ref.shape)
    print('Current Data Shape: ',cur.shape)
